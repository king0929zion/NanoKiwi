...
const SidePanel = () => {
          ...
          switch (state) {
            case ExecutionState.TASK_START:
              ...
              if (currentTabIdRef.current) {
                chrome.tabs.sendMessage(currentTabIdRef.current, {
                  type: 'show_wiki_overlay',
                }).catch(err => console.error('Failed to show overlay:', err));
              }
              break;
            ...
            case ExecutionState.TASK_PAUSE:
              setIsTaskRunning(false);
              // Hide overlay but keep button visible
              if (currentTabIdRef.current) {
                chrome.tabs.sendMessage(currentTabIdRef.current, {
                  type: 'hide_wiki_overlay',
                }).catch(err => console.error('Failed to hide overlay:', err));
  ...
  return (
    <div>
      <div
        className={`flex h-screen flex-col ${isDarkMode ? 'bg-[#1A1A1A]' : 'bg-[#FBFBF9]'} overflow-hidden border ${isDarkMode ? 'border-[#404040]' : 'border-[#E5E5E0]'} rounded-2xl`}>
          ...
          <div className="header-logo">
            {showHistory ? (
              <button
                ...
                className={`${isDarkMode ? 'text-gray-400 hover:text-gray-300' : 'text-gray-600 hover:text-gray-700'} cursor-pointer`}
            ...
            {!showHistory && (
              <>
                <button
                  ...
                  className={`header-icon ${isDarkMode ? 'text-gray-400 hover:text-gray-300' : 'text-gray-600 hover:text-gray-700'} cursor-pointer`}
        ...
        {showHistory ? (
          ...
          <>
            ...
            {hasConfiguredModels === null && (
              <div
                ...
                <div className="text-center">
                  <div className={`mx-auto mb-4 size-8 animate-spin rounded-full border-2 ${isDarkMode ? 'border-gray-500 border-t-gray-300' : 'border-gray-400 border-t-gray-600'}`}></div>
            ...
            {hasConfiguredModels === false && (
              <div
                className={`flex flex-1 items-center justify-center p-8 ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                <div className="max-w-md text-center">
                  <img src="/icon-128.png" alt="Wiki Logo" className="mx-auto mb-4 size-12" />
                  <h3 className={`mb-2 text-lg font-semibold ${isDarkMode ? 'text-gray-200' : 'text-gray-900'}`}>
                    {t('welcome_title')}
                  </h3>
                  <p className={`mb-4 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>{t('welcome_instruction')}</p>
                  <button
                    onClick={() => chrome.runtime.openOptionsPage()}
                    className={`my-4 rounded-lg px-4 py-2 font-medium transition-colors ${
                      isDarkMode ? 'bg-[#D97706] text-white hover:bg-[#B45309]' : 'bg-[#F59E0B] text-white hover:bg-[#D97706]'
                    }`}>
            ...
            {hasConfiguredModels === true && (
              <>
                {messages.length === 0 && (
                  <>
                    <div
                      className={`border-t ${isDarkMode ? 'border-[#404040]' : 'border-[#E5E5E0]'} mb-2 p-2 shadow-sm backdrop-blur-sm`}>
                      <ChatInput
                ...
                {messages.length > 0 && (
                  <div
                    className={`scrollbar-gutter-stable flex-1 overflow-x-hidden overflow-y-scroll scroll-smooth p-2 ${isDarkMode ? 'bg-[#1A1A1A]' : 'bg-[#FBFBF9]'}`}>
