...
Wiki is an open-source AI web automation Chrome extension that runs multi-agent systems locally in the browser. It's a free alternative to OpenAI Operator with support for multiple LLM providers (OpenAI, Anthropic, Gemini, Ollama, etc.).
...
- `pnpm -F pages/side-panel build` 鈥?build only the side panel
- `pnpm -F chrome-extension dev` 鈥?dev-watch background/service worker
- `pnpm -F packages/storage type-check` 鈥?TS checks for storage package
- `pnpm -F pages/side-panel lint -- src/components/ChatInput.tsx` 鈥?lint a file
- `pnpm -F chrome-extension prettier -- src/background/index.ts` 鈥?format a file
...
- **Error Handling**: Avoid exposing sensitive information in error messages or logs
 - **Secrets/Config**: Use `.env.local` (git鈥慽gnored) and prefix variables with `VITE_`.
...
- Vite aliases: pages use `@src` for page `src/`; the extension uses
  ...
  `packages/vite-config`鈥檚 `withPageConfig` for page workspaces.
 ...
 - Change policy: ask first for new deps, file renames/moves/deletes, or
   ...
   workspace鈥憇coped lint/format/type-check/build, and small focused patches
